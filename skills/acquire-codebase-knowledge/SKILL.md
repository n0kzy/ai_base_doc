...
Produces seven populated documents in `docs/codebase/` covering everything needed to work effectively on the project. Only document what is verifiable from files or terminal output â€” never infer or assume.
...
1. [STACK.md](assets/templates/STACK.md) â€” language, runtime, frameworks, all dependencies
2. [STRUCTURE.md](assets/templates/STRUCTURE.md) â€” directory layout, entry points, key files
3. [ARCHITECTURE.md](assets/templates/ARCHITECTURE.md) â€” layers, patterns, data flow
4. [CONVENTIONS.md](assets/templates/CONVENTIONS.md) â€” naming, formatting, error handling, imports
5. [INTEGRATIONS.md](assets/templates/INTEGRATIONS.md) â€” external APIs, databases, auth, monitoring
6. [TESTING.md](assets/templates/TESTING.md) â€” frameworks, file organization, mocking strategy
7. [CONCERNS.md](assets/templates/CONCERNS.md) â€” tech debt, bugs, security risks, perf bottlenecks
...
**Monorepos:** Root `package.json` may have no source â€” check for `workspaces`, `packages/`, or `apps/` directories. Each workspace may have independent dependencies and conventions. Map each sub-package separately.
...
**Generated/compiled output:** Never document patterns from `dist/`, `build/`, `generated/`, `.next/`, `out/`, or `__pycache__/`. These are artefacts â€” document source conventions only.
...
**`devDependencies` â‰  production stack:** Only `dependencies` (or equivalent, e.g. `[tool.poetry.dependencies]`) runs in production. Document linters, formatters, and test frameworks separately as dev tooling.

**Test TODOs â‰  production debt:** TODOs inside `test/`, `tests/`, `__tests__/`, or `spec/` are coverage gaps, not production technical debt. Separate them in `CONCERNS.md`.
...
- **CODE METRICS** â€” Total files, lines of code by language, largest files (complexity signals)
- **CI/CD PIPELINES** â€” Detected GitHub Actions, GitLab CI, Jenkins, CircleCI, etc.
- **CONTAINERS & ORCHESTRATION** â€” Docker, Docker Compose, Kubernetes, Vagrant configs
- **SECURITY & COMPLIANCE** â€” Snyk, Dependabot, SECURITY.md, SBOM, security policies
- **PERFORMANCE & TESTING** â€” Benchmark configs, profiling markers, load testing tools
...
| Asset | When to load |
|-------|-------------|
| [`scripts/scan.py`](scripts/scan.py) | Phase 1 â€” run first, before reading any code (Python 3.8+ required) |

| [`references/inquiry-checkpoints.md`](references/inquiry-checkpoints.md) | Phase 2 â€” load for per-template investigation questions |
| [`references/stack-detection.md`](references/stack-detection.md) | Phase 2 â€” only if stack is ambiguous |
