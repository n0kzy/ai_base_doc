...
## ðŸŽ¯ PostgreSQL-Specific Review Areas

### JSONB Best Practices
```sql
-- âŒ BAD: Inefficient JSONB usage
SELECT * FROM orders WHERE data->>'status' = 'shipped';  -- No index support

-- âœ… GOOD: Indexable JSONB queries
CREATE INDEX idx_orders_status ON orders USING gin((data->'status'));
SELECT * FROM orders WHERE data @> '{"status": "shipped"}';

-- âŒ BAD: Deep nesting without consideration
UPDATE orders SET data = data || '{"shipping":{"tracking":{"number":"123"}}}';

-- âœ… GOOD: Structured JSONB with validation
...
```sql
-- âŒ BAD: Inefficient array operations
SELECT * FROM products WHERE 'electronics' = ANY(categories);  -- No index

-- âœ… GOOD: GIN indexed array queries
CREATE INDEX idx_products_categories ON products USING gin(categories);
SELECT * FROM products WHERE categories @> ARRAY['electronics'];

-- âŒ BAD: Array concatenation in loops
-- This would be inefficient in a function/procedure

-- âœ… GOOD: Bulk array operations
...
```sql
-- âŒ BAD: Not using PostgreSQL features
CREATE TABLE users (
    id INTEGER,
    email VARCHAR(255),
...
-- âœ… GOOD: PostgreSQL-optimized schema
CREATE TABLE users (
...
```sql
-- âŒ BAD: Using generic types for specific data
...
-- âœ… GOOD: PostgreSQL custom types
...
## ðŸ” PostgreSQL-Specific Anti-Patterns
...
```sql
-- âŒ BAD: Inefficient trigger function
...
-- âœ… GOOD: Optimized trigger function
CREATE OR REPLACE FUNCTION update_modified_time()
RETURNS TRIGGER AS $$
BEGIN
...
## ðŸ“Š PostgreSQL Extension Usage Review

### Extension Best Practices
```sql
-- âœ… Check if extension exists before creating
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS "pgcrypto";
CREATE EXTENSION IF NOT EXISTS "pg_trgm";

-- âœ… Use extensions appropriately
-- UUID generation
...
## ðŸ›¡ï¸ PostgreSQL Security Review

### Row Level Security (RLS)
```sql
-- âœ… GOOD: Implementing RLS
...
```sql
-- âŒ BAD: Overly broad permissions
...
## ðŸŽ¯ PostgreSQL Code Quality Checklist
...
## ðŸ“ PostgreSQL-Specific Review Guidelines
